      ...
      <div className={classes.container}>
        <AnimatePresence>
          {messages.map((message) => (
            ...
            >
              {message.type === 'ai' && (
                <div className={`${classes.avatarSize} rounded-full bg-primary flex items-center justify-center flex-shrink-0`}>
                  <Bot className={`${classes.iconSize} text-white`} />
                </div>
              )}
              
              <div className={`flex flex-col ${classes.maxWidth} ${classes.gap}`}>
                <div
                ...
                >
                  <p className={`whitespace-pre-wrap ${classes.textSize} leading-relaxed`}>
                    {message.content}
                  </p>
                ...
                {message.type === 'ai' && message.videoSuggestions && message.videoSuggestions.length > 0 && (
                  <div className={`space-y-2 ${deviceType === 'mobile' ? 'space-y-1.5' : 'space-y-2'}`}>
                    <p className={`${classes.timestampSize} text-muted-foreground font-medium px-1`}>
                      📺 Related Videos:
                    </p>
              ...
              {message.type === 'user' && (
                <div className={`${classes.avatarSize} rounded-full bg-secondary flex items-center justify-center flex-shrink-0`}>
                  <User className={`${classes.iconSize} text-secondary-foreground`} />
                </div>
              )}
            </motion.div>
          ))}
