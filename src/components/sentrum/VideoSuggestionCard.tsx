...
const VideoSuggestionCard: React.FC<VideoSuggestionCardProps> = ({ video, index, deviceType = 'desktop' }) => {
  ...
  return (
    ...
    >
      <div className={`flex items-start ${classes.gap}`}>
        <div className={`${classes.iconContainer} bg-primary/10 rounded-lg flex items-center justify-center flex-shrink-0`}>
          <Play className={`${classes.iconSize} text-primary`} />
        </div>
        
        <div className="flex-1 min-w-0">
          <h4 className={`font-medium ${classes.titleSize} text-foreground line-clamp-2 mb-1`}>
            {video.title}
          </h4>
          <div className={`flex items-center gap-2 ${classes.categorySize} text-muted-foreground`}>
            <span className={`bg-background/80 px-2 py-0.5 rounded-full ${classes.categorySize}`}>
              {video.category}
            </span>
