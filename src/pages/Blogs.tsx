...
const blogPosts = [
  ...
  },
  {
    ...
    excerpt: "Beginner's guide to setting up and using MongoDB in your web projects.",
    date: "2024-04-10",
...
const Blogs: React.FC = () => {
  return (
    <div className="max-w-7xl mx-auto py-12 px-2 sm:px-4 md:px-8">
      ...
      <p className="text-lg text-muted-foreground mb-10">Read the latest articles, guides, and tutorials.</p>
      <motion.div
      ...
      >
        {blogPosts.map((post) => (
          <motion.div key={post.slug} variants={cardVariants} className="h-full group">
            <MagneticCard intensity={0.2} scale={1.06} className="h-full">
              <Link to={`/${post.slug}`} className="no-underline text-inherit h-full block focus:outline-none">
                <Card className="h-full border border-border hover:border-primary/30 transition-all duration-500 hover:shadow-2xl bg-card/80 backdrop-blur-sm cursor-pointer flex flex-col">
                  ...
                  <CardContent className="flex-1 flex flex-col text-center p-6 pt-0">
                    <CardTitle className="mb-2 text-lg font-bold line-clamp-2">{post.title}</CardTitle>
                    <CardDescription className="mb-4 flex-grow line-clamp-3">{post.excerpt}</CardDescription>
                    ...
                    <div className="text-xs text-muted-foreground mt-auto">{new Date(post.date).toLocaleDateString()}</div>
                  </CardContent>
                </Card>
              </Link>
            </MagneticCard>
