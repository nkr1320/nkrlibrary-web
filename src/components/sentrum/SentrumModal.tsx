...
const SentrumModal: React.FC = () => {
  ...
  const getModalClasses = () => {
    const baseClasses = "fixed z-50 bg-background border border-border shadow-2xl flex flex-col rounded-xl";
            ...
            <div className="flex items-center justify-between p-3 sm:p-4 border-b border-border">
              <div className="flex items-center gap-1">
                {views.map((view) => (
                  <Button
                    key={view.id}
                    variant={state.currentView === view.id ? 'default' : 'ghost'}
                    size={deviceType === 'mobile' ? 'sm' : 'sm'}
                    onClick={() => setCurrentView(view.id as 'chat' | 'resume' | 'notebook')}
                    className={`flex items-center gap-2 ${
                      deviceType === 'mobile' ? 'px-2 py-1.5' : 'px-3 py-2'
                    }`}
                  >
                    <view.icon className={`${deviceType === 'mobile' ? 'h-3.5 w-3.5' : 'h-4 w-4'}`} />
                    <span className={`${deviceType === 'mobile' ? 'text-xs' : 'text-sm'} hidden sm:inline`}>{view.label}</span>
                  </Button>
                ))}
              </div>
              
              <Button onClick={closeSentrum} variant="ghost" size="sm">
                <X className="h-4 w-4" />
              </Button>
            </div>
