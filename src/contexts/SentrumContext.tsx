...
export const SentrumProvider: React.FC<SentrumProviderProps> = ({
  ...
  const openSentrum = useCallback(
    (view: "chat" | "resume" | "notebook" = "chat") => {
  ...
  const getTopicContent = useCallback((topic: string) => {
    ...
    if (
      ...
      topicLower.includes("phishing") ||
      topicLower.includes("malware") ||
      topicLower.includes("hack") ||
      topicLower.includes("fraud") ||
      topicLower.includes("sextortion") ||
      topicLower.includes("honey trap") ||
      topicLower.includes("data breach") ||
      topicLower.includes("safety") ||
      topicLower.includes("attack")
    ) {
      const cyberVideos = getCriticalVideos().filter(
        (v) => v.category === "CyberScams",
      );
      const scamTypes = [
    ...
    if (
      ...
      topicLower.includes("icd") ||
      topicLower.includes("diagnosis") ||
      topicLower.includes("chart")
    ) {
      const medicalVideos = getCriticalVideos().filter(
    ...
    if (
      ...
      topicLower.includes("git") ||
      topicLower.includes("development") ||
      topicLower.includes("tutorial")
    ) {
      const softwareVideos = getCriticalVideos().filter(
        (v) => v.category === "Software",
      );
      return {
        type: "software",
  ...
  const generateAIResponse = useCallback(
    (userMessage: string): { content: string; videoSuggestions: Video[] } => {
      ...
      if (lowerMessage.includes("resume") || lowerMessage.includes("cv")) {
        ...
        if (
          ...
          const positionMatch = lowerMessage.match(
            /as (a |an )?([A-Za-z\s]+?) (at|in)/,
          );

          let response = "🎯 **ADDING WORK EXPERIENCE**\n\n";
          response +=
            "I'll help you add your work experience to your resume!\n\n";

          if (companyMatch || positionMatch) {
            response += "**Detected Information:**\n";
            if (companyMatch)
              response += `• Company: ${companyMatch[1].trim()}\n`;
            if (positionMatch)
              response += `• Position: ${positionMatch[2].trim()}\n`;
            response += "\n";
          }
        ...
        if (lowerMessage.includes("add") && lowerMessage.includes("skill")) {
          ...
          response +=
            "• **Industry-Specific:** Domain knowledge, certifications\n\n";
        ...
        if (
          ...
          response +=
            "💡 **Tip:** List education in reverse chronological order!";
          return { content: response, videoSuggestions: [] };
        }
        ...
        if (
          ...
          response +=
            "I'll help you craft a compelling professional summary!\n\n";
          ...
          response +=
            '"[X] years experienced [role] with expertise in [skills/areas]. Proven track record in [achievements]. Seeking to leverage [skills] to [value for employer]."\n\n';
          response +=
            "💡 **Pro tip:** Tailor your summary to each job application!";
          return { content: response, videoSuggestions: [] };
        }
        ...
        if (
          ...
          response +=
            "**📊 Your Current ATS Score:** Check the main resume builder for live scoring!\n\n";
          response +=
            "💡 **Pro tip:** Mirror the language used in job postings!";
          return { content: response, videoSuggestions: [] };
        }
        ...
        response +=
          "I'm here to help you create an outstanding resume! Here's what I can help with:\n\n";
      ...
      if (searchResults.length > 0) {
        videoSuggestions = searchResults
          .slice(0, 3)
      ...
      if (topicContent) {
        ...
        if (topicContent.type === "cybersecurity") {
          ...
          return {
            content: response,
            videoSuggestions: topicContent.videos.slice(0, 3),
          };
        }
      ...
      if (
        ...
        if (lowerMessage.includes("cyber") || lowerMessage.includes("scam")) {
          ...
          return {
            content:
              "I'll take you to the Cyber Scams section where you can learn about staying safe online! 🛡️",
            videoSuggestions,
          };
        }
        if (lowerMessage.includes("about")) {
          ...
          return {
            content:
              "Navigating to the About page to learn more about NKR Library! 📖",
            videoSuggestions,
          };
        }
        if (lowerMessage.includes("contact")) {
          ...
          return {
            content: "Taking you to the Contact page! 📞",
            videoSuggestions,
          };
        }
        if (
          lowerMessage.includes("videos") ||
          lowerMessage.includes("tutorial")
        ) {
          setTimeout(() => navigateToPage("/videos"), 1000);
          return {
            content: "Opening the Videos section with all our tutorials! 🎥",
            videoSuggestions,
          };
        }
        if (lowerMessage.includes("courses")) {
          ...
          return {
            content: "Redirecting to our Courses page! 🎓",
            videoSuggestions,
          };
        }
      }
      ...
      if (
        ...
        if (searchResults.length > 0) {
          ...
          return {
            content: response,
            videoSuggestions: topResults.slice(0, 3).map((r) => r.video),
          };
        }
        return {
          content:
            "I couldn't find specific content matching your query. Try asking about 'cybersecurity', 'medical coding', 'web development', or other topics! 🔍",
          videoSuggestions: [],
        };
      }
      ...
      if (lowerMessage.includes("resume") || lowerMessage.includes("cv")) {
        return {
          content:
            "I can help you build a professional resume! Click the Resume Builder tab, and I'll guide you through creating a modern, ATS-friendly resume. 📄✨",
          videoSuggestions,
        };
      }
      ...
      if (
        ...
        const sessionTime = Math.round(
          (Date.now() - state.currentContext.sessionStartTime.getTime()) /
            (1000 * 60),
        );
        return {
  ...
  const addNote = useCallback(
    (content: string, tags: string[], videoId?: string) => {
  ...
  const getSummary = useCallback(() => {
    const { visitedPages, watchedVideos, sessionStartTime } =
